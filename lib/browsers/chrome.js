...
chrome.closeTab = function (tab) {
  return new Promise((resolve, reject) => {
    // Signal to plugins/interceptors that this tab is closing to avoid sending further protocol commands
    try {
      tab._isClosing = true;
    } catch (e) {}
...
chrome.setUpEvents = async function (tab) {
  ...
  Network.requestWillBeSent((params) => {
    // Skip tracking blob URLs as they don't make actual network requests
    if (params.request.url.startsWith('blob:')) {
      if (tab.prerender.logRequests || this.options.logRequests)
        util.log('Skipping blob URL:', params.request.url);
      return;
    }

    tab.prerender.numRequestsInFlight++;
    tab.prerender.requests[params.requestId] = params.request.url;
    tab.prerender.requestStartTimes = tab.prerender.requestStartTimes || {};
    tab.prerender.requestStartTimes[params.requestId] = params.timestamp * 1000;
    if (tab.prerender.logRequests || this.options.logRequests)
  ...
  Network.loadingFinished(({ requestId, timestamp, encodedDataLength }) => {
    ...
    if (request) {
      // Skip blob URLs (shouldn't happen now, but defensive programming)
      if (request.startsWith('blob:')) {
        delete tab.prerender.requests[requestId];
        delete tab.prerender.requestStartTimes[requestId];
        return;
      }
      ...
      const startTime = tab.prerender.requestStartTimes[requestId];
      const duration = startTime ? timestamp * 1000 - startTime : 0;
  ...
  Network.loadingFailed((params) => {
    if (tab.prerender.requests[params.requestId]) {
      // Skip blob URLs (shouldn't happen now, but defensive programming)
      if (tab.prerender.requests[params.requestId].startsWith('blob:')) {
        delete tab.prerender.requests[params.requestId];
        delete tab.prerender.requestStartTimes[params.requestId];
        return;
      }
      ...
      const startTime = tab.prerender.requestStartTimes[params.requestId];
      const duration = startTime ? params.timestamp * 1000 - startTime : 0;
  ...
  return new Promise((resolve, reject) => {
    ...
    tab.Runtime.evaluate({
      ...
      .then((result) => {
        ...
        if (!done) {
          util.log(
            'Page not done loading. In flight:',
            tab.prerender.numRequestsInFlight,
            'lastRequestAt:',
            new Date(tab.prerender.lastRequestReceivedAt).toISOString(),
            'waitAfterLastRequest:',
            waitAfterLastRequest,
          );
          if (tab.prerender.numRequestsInFlight > 0) {
            const inFlightUrls = Object.values(tab.prerender.requests);
            util.log('In-flight requests:', inFlightUrls);
          }
        }
