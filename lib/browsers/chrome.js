...
chrome.spawn = function (options) {
  return new Promise((resolve, reject) => {
    ...
    // Build chrome flags if not explicitly provided. Preserve ability to add
    // extraChromeFlags/proxyServer (local enhancement) while adopting the
    // upstream PR's detached process group strategy for improved shutdown.
    ...
    // Use a detached process group so we can terminate all spawned children
    // reliably (from upstream PR 779). If the user supplied full flags array
    // via options.chromeFlags we still pass them directly (above).
...
chrome.kill = function () {
  if (this.chromeChild) {
    ...
    try {
      // negative PID means send to the entire process group (detached spawn)
      process.kill(-this.chromeChild.pid, signal);
    } catch (e) {
      util.log('Error killing Chrome process group, falling back to direct kill', e);
      try { this.chromeChild.kill(signal); } catch (_) {}
    }
  }
};
...
chrome.connect = function () {
  return new Promise((resolve, reject) => {
    ...
    let connect = () => {
      const debugHost = this.options.browserDebuggingHost || host;
      CDP.Version({ host: debugHost, port: this.options.browserDebuggingPort })
        .then((info) => {
          this.originalUserAgent = info['User-Agent'];
          this.webSocketDebuggerURL =
            info.webSocketDebuggerUrl ||
            `ws://${debugHost}:${this.options.browserDebuggingPort}/devtools/browser`;
...
chrome.openTab = function (options) {
  return new Promise((resolve, reject) => {
    ...
    const connectToBrowser = async (debugHost, port, target) => {
      let remainingRetries = 5;
      for (;;) {
        try {
          return await CDP({ host: debugHost, port, target });
        } catch (err) {
          util.log(
            `Cannot connect to browser host=${debugHost}, port=${port}, target=${target}, remainingRetries=${remainingRetries}, url=${url}`,
            err,
          );
